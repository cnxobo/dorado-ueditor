...
var CodeMirror = (function () {
    function CodeMirror(place, givenOptions) {
        ...
        wrapper.innerHTML = "<div style=\"overflow: hidden; position: relative; width: 3px; height: 0px;\">" + "<textarea style=\"position: absolute; padding: 0; width: 1px;\" wrap=\"off\" " + "autocorrect=\"off\" autocapitalize=\"off\"></textarea></div>" + "<div class=\"CodeMirror-scroll\" tabindex=\"-1\">" + "<div style=\"position: relative\">" + "<div style=\"position: relative\">" + "<div class=\"CodeMirror-gutter\"><div class=\"CodeMirror-gutter-text\"></div></div>" + "<div class=\"CodeMirror-lines\"><div style=\"position: relative\">" + "<div style=\"position: absolute; width: 100%; height: 0; overflow: hidden; visibility: hidden\"></div>" + "<pre class=\"CodeMirror-cursor\">&#160;</pre>" + "<div></div>" + "</div></div></div></div></div>";
        ...
        var instance = wrapper.CodeMirror = {getValue:getValue, setValue:operation(setValue), getSelection:getSelection, replaceSelection:operation(replaceSelection), focus:function () {
            focusInput();
            onFocus();
            fastPoll();
        ...
        }, undo:operation(undo), redo:operation(redo), indentLine:operation(function (n, dir) {
            if (isLine(n)) {
                indentLine(n, dir == null ? "smart" : dir ? "add" : "subtract");
            }
        }), indentSelection:operation(indentSelected), historySize:function () {
        ...
        }, markText:operation(markText), setBookmark:setBookmark, setMarker:operation(addGutterMarker), clearMarker:operation(removeGutterMarker), setLineClass:operation(setLineClass), hideLine:operation(function (h) {
            ...
            return line;
        }, lineInfo:lineInfo, addWidget:function (pos, node, scroll, vert, horiz) {
            ...
            if (vert == "over") {
                ...
                if (vert == "near") {
                    var vspace = Math.max(scroller.offsetHeight, doc.height * textHeight()), hspace = Math.max(code.clientWidth, lineSpace.clientWidth) - paddingLeft();
            ...
            return doc.size;
        }, clipPos:clipPos, getCursor:function (start) {
            if (start == null) {
                start = sel.inverted;
            }
            ...
            if (isLine(line)) {
                replaceRange(text, {line:line, ch:0}, {line:line, ch:getLine(line).text.length});
            }
        }), removeLine:operation(function (line) {
            if (isLine(line)) {
                replaceRange("", {line:line, ch:0}, clipPos({line:line + 1, ch:0}));
            }
        }), replaceRange:operation(replaceRange), getRange:function (from, to) {
            return getRange(clipPos(from), clipPos(to));
        }, execCommand:function (cmd) {
            return commands[cmd](instance);
        }, moveH:operation(moveH), deleteH:operation(deleteH), moveV:operation(moveV), toggleOverwrite:function () {
            overwrite = !overwrite;
        ...
        function onKeyPress(e) {
            ...
            if (window.opera && !e.which && handleKeyBinding(e)) {
                return;
            }
        ...
        var prevInput = "";
        function readInput() {
            if (leaveInputAlone || !focused || hasSelection(input)) {
                return false;
            }
        ...
        function setSelection(from, to, oldFrom, oldTo) {
            ...
            if (from.line != oldFrom) {
                from = skipHidden(from, oldFrom, sel.from.ch);
            }
            if (to.line != oldTo) {
        ...
        function skipHidden(pos, oldLine, oldCh) {
            function getNonHidden(dir) {
                ...
                while (lNo != end) {
                    ...
                    if (!line.hidden) {
                        var ch = pos.ch;
                        if (ch > oldCh || ch > line.text.length) {
                            ch = line.text.length;
                        }
        ...
        function clipPos(pos) {
            ...
            if (pos.line >= doc.size) {
                return {line:doc.size - 1, ch:getLine(doc.size - 1).text.length};
            }
        ...
        function indentLine(n, how) {
            ...
            var line = getLine(n), curSpace = line.indentation(options.tabSize), curSpaceString = line.text.match(/^\s*/)[0], indentation;
            if (how == "prev") {
                if (n) {
        ...
        function computeTabText() {
            for (var str = "<span class=\"cm-tab\">", i = 0; i < options.tabSize; ++i) {
                str += " ";
            }
            return str + "</span>";
        }
                ...
                for (var j = 0; j < mk.length; ++j) {
                    ...
                    if (mark.set == this.set) {
                        if (mark.from != null || mark.to != null) {
                            ...
                            if (found != null) {
                                if (mark.from != null) {
                                    from = {line:found, ch:mark.from};
                                }
                                if (mark.to != null) {
                                    to = {line:found, ch:mark.to};
                                }
                            }
                        }
                    }
            ...
            return changeLine(handle, function (line, no) {
                if (line.hidden != hidden) {
                    ...
                    if (hidden && (sel.from.line == no || sel.to.line == no)) {
                        setSelection(skipHidden(sel.from, sel.from.line, sel.from.ch), skipHidden(sel.to, sel.to.line, sel.to.ch));
                    }
        ...
        var tempId = Math.floor(Math.random() * 16777215).toString(16);
        ...
        function coordsChar(x, y) {
            ...
            for (; ; ) {
                if (to - from <= 1) {
                    return {line:lineNo, ch:(toX - x > x - fromX) ? from : to};
                }
                var middle = Math.ceil((from + to) / 2), middleX = getX(middle);
                if (middleX > x) {
                    to = middle;
        ...
        function matchBrackets(autoclear) {
            ...
            var one = markText({line:head.line, ch:pos}, {line:head.line, ch:pos + 1}, style), two = found.pos != null && markText({line:i, ch:found.pos}, {line:i, ch:found.pos + 1}, style);
        ...
        function highlightWorker() {
            ...
            while (work.length) {
                ...
                var unchanged = 0, compare = mode.compareStates, realChange = false, i = start, bail = false;
        ...
        function endOperation() {
            ...
            var tc = textChanged, cbs = callbacks;
    ...
    var commands = CodeMirror.commands = {selectAll:function (cm) {
        ...
        if (cur.ch > 0 && cur.ch < line.length - 1) {
            cm.replaceRange(line.charAt(cur.ch) + line.charAt(cur.ch - 1), {line:cur.line, ch:cur.ch - 1}, {line:cur.line, ch:cur.ch + 1});
        }
    ...
    keyMap.basic = {"Left":"goCharLeft", "Right":"goCharRight", "Up":"goLineUp", "Down":"goLineDown", "End":"goLineEnd", "Home":"goLineStartSmart", "PageUp":"goPageUp", "PageDown":"goPageDown", "Delete":"delCharRight", "Backspace":"delCharLeft", "Tab":"indentMore", "Shift-Tab":"indentLess", "Enter":"newlineAndIndent", "Insert":"toggleOverwrite"};
    keyMap.pcDefault = {"Ctrl-A":"selectAll", "Ctrl-D":"deleteLine", "Ctrl-Z":"undo", "Shift-Ctrl-Z":"redo", "Ctrl-Y":"redo", "Ctrl-Home":"goDocStart", "Alt-Up":"goDocStart", "Ctrl-End":"goDocEnd", "Ctrl-Down":"goDocEnd", "Ctrl-Left":"goWordLeft", "Ctrl-Right":"goWordRight", "Alt-Left":"goLineStart", "Alt-Right":"goLineEnd", "Ctrl-Backspace":"delWordLeft", "Ctrl-Delete":"delWordRight", "Ctrl-S":"save", "Ctrl-F":"find", "Ctrl-G":"findNext", "Shift-Ctrl-G":"findPrev", "Shift-Ctrl-F":"replace", "Shift-Ctrl-R":"replaceAll", fallthrough:"basic"};
    keyMap.macDefault = {"Cmd-A":"selectAll", "Cmd-D":"deleteLine", "Cmd-Z":"undo", "Shift-Cmd-Z":"redo", "Cmd-Y":"redo", "Cmd-Up":"goDocStart", "Cmd-End":"goDocEnd", "Cmd-Down":"goDocEnd", "Alt-Left":"goWordLeft", "Alt-Right":"goWordRight", "Cmd-Left":"goLineStart", "Cmd-Right":"goLineEnd", "Alt-Backspace":"delWordLeft", "Ctrl-Alt-Backspace":"delWordRight", "Alt-Delete":"delWordRight", "Cmd-S":"save", "Cmd-F":"find", "Cmd-G":"findNext", "Shift-Cmd-G":"findPrev", "Cmd-Alt-F":"replace", "Shift-Cmd-Alt-F":"replaceAll", fallthrough:["basic", "emacsy"]};
    ...
    Line.prototype = {replace:function (from, to_, text) {
        ...
        function span(text, style) {
            ...
            if (first && ie && text.charAt(0) == " ") {
                text = "\xa0" + text.slice(1);
            }
            first = false;
            if (style) {
                html.push("<span class=\"", style, "\">", htmlEscape(text).replace(/\t/g, tabText), "</span>");
            } else {
                html.push(htmlEscape(text).replace(/\t/g, tabText));
            }
        }
...
CodeMirror.defineMode("xml", function (config, parserConfig) {
    ...
    var Kludges = parserConfig.htmlMode ? {autoSelfClosers:{"br":true, "img":true, "hr":true, "link":true, "input":true, "meta":true, "col":true, "frame":true, "base":true, "area":true}, doNotIndent:{"pre":true}, allowUnquoted:true} : {autoSelfClosers:{}, doNotIndent:{}, allowUnquoted:false};
...
CodeMirror.defineMode("javascript", function (config, parserConfig) {
    ...
    return {startState:function (basecolumn) {
        return {tokenize:jsTokenBase, reAllowed:true, kwAllowed:true, cc:[], lexical:new JSLexical((basecolumn || 0) - indentUnit, 0, "block", false), localVars:null, context:null, indented:0};
    }, token:function (stream, state) {
        if (stream.sol()) {
            if (!state.lexical.hasOwnProperty("align")) {
