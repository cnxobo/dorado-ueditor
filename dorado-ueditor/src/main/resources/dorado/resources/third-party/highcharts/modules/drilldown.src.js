...
(function (H) {
    "use strict";
    var noop = function () {
    }, defaultOptions = H.getOptions(), each = H.each, extend = H.extend, wrap = H.wrap, Chart = H.Chart, seriesTypes = H.seriesTypes, PieSeries = seriesTypes.pie, ColumnSeries = seriesTypes.column, fireEvent = HighchartsAdapter.fireEvent;
    ...
    extend(defaultOptions.lang, {drillUpText:"\u25c1 Back to {series.name}"});
    defaultOptions.drilldown = {activeAxisLabelStyle:{cursor:"pointer", color:"#039", fontWeight:"bold", textDecoration:"underline"}, activeDataLabelStyle:{cursor:"pointer", color:"#039", fontWeight:"bold", textDecoration:"underline"}, animation:{duration:500}, drillUpButton:{position:{align:"right", x:-10, y:10}}};
    ...
    Chart.prototype.addSeriesAsDrilldown = function (point, ddOptions) {
        var oldSeries = point.series, xAxis = oldSeries.xAxis, yAxis = oldSeries.yAxis, newSeries, color = point.color || oldSeries.color, pointIndex, level;
        ddOptions = extend({color:color}, ddOptions);
    ...
    Chart.prototype.showDrillUpButton = function () {
        var chart = this, backText = this.getDrilldownBackText(), buttonOptions = chart.options.drilldown.drillUpButton;
        if (!this.drillUpButton) {
            this.drillUpButton = this.renderer.button(backText, null, null, function () {
                chart.drillUp();
            }).attr(extend({align:buttonOptions.position.align, zIndex:9}, buttonOptions.theme)).add().align(buttonOptions.position, false, buttonOptions.relativeTo || "plotBox");
        } else {
            this.drillUpButton.attr({text:backText}).align();
        }
    };
    Chart.prototype.drillUp = function () {
        var chart = this, level = chart.drilldownLevels.pop(), oldSeries = chart.series[0], oldExtremes = level.oldExtremes, newSeries = chart.addSeries(level.seriesOptions, false);
    ...
    PieSeries.prototype.animateDrilldown = function (init) {
        ...
        if (!init) {
            each(this.points, function (point, i) {
                ...
                point.graphic.attr(H.merge(animateFrom, {start:start + i * startAngle, end:start + (i + 1) * startAngle})).animate(point.shapeArgs, H.merge(animationOptions, {step:function (val, fx) {
                    if (fx.prop === "start") {
                        this.attr({fill:tweenColors(startColor, endColor, fx.pos)});
                    }
                }}));
            });
        }
    ...
    ColumnSeries.prototype.animateDrilldown = function (init) {
        var animateFrom = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1].shapeArgs, animationOptions = this.chart.options.drilldown.animation;
        if (!init) {
    ...
    ColumnSeries.prototype.animateDrillupFrom = PieSeries.prototype.animateDrillupFrom = function (level) {
        var animationOptions = this.chart.options.drilldown.animation, group = this.group;
        delete this.group;
        each(this.points, function (point) {
            var graphic = point.graphic, startColor = H.Color(point.color).rgba;
            delete point.graphic;
            graphic.animate(level.shapeArgs, H.merge(animationOptions, {step:function (val, fx) {
                if (fx.prop === "start") {
                    this.attr({fill:tweenColors(startColor, H.Color(level.color).rgba, fx.pos)});
                }
            }, complete:function () {
                graphic.destroy();
                if (group) {
    ...
    wrap(H.Point.prototype, "init", function (proceed, series, options, x) {
        ...
        if (point.drilldown) {
            ...
            if (tickLabel && tickLabel._basicStyle) {
    ...
    wrap(H.Series.prototype, "drawDataLabels", function (proceed) {
        ...
        each(this.points, function (point) {
            if (point.drilldown && point.dataLabel) {
                point.dataLabel.attr({"class":"highcharts-drilldown-data-label"}).css(css).on("click", function () {
                    point.doDrilldown();
                });
            }
        });
    ...
    var type, drawTrackerWrapper = function (proceed) {
        ...
        each(this.points, function (point) {
            if (point.drilldown && point.graphic) {
                point.graphic.attr({"class":"highcharts-drilldown-point"}).css({cursor:"pointer"});
            }
        });
    };
