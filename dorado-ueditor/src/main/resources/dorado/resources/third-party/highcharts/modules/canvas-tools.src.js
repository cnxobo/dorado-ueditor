...
(function () {
    ...
    function build() {
        ...
        svg.Property = function (name, value) {
            ...
            this.Length = {DPI:function (viewPort) {
                return 96;
                ...
                if (s.match(/ex$/)) {
                    return that.numValue() * this.EM(viewPort) / 2;
                }
                if (s.match(/px$/)) {
            ...
            this.Angle = {toRadians:function () {
                ...
                var s = that.value + "";
                if (s.match(/deg$/)) {
                    return that.numValue() * (Math.PI / 180);
                }
                if (s.match(/grad$/)) {
                    return that.numValue() * (Math.PI / 200);
                }
                if (s.match(/rad$/)) {
                    return that.numValue();
                }
            ...
            this.Parse = function (s) {
                ...
                for (var i = 0; i < d.length; i++) {
                    if (!set.fontStyle && that.Styles.indexOf(d[i]) != -1) {
                        ...
                        if (!set.fontVariant && that.Variants.indexOf(d[i]) != -1) {
                            if (d[i] != "inherit") {
                                f.fontVariant = d[i];
                            }
                            set.fontStyle = set.fontVariant = true;
                        } else {
                            if (!set.fontWeight && that.Weights.indexOf(d[i]) != -1) {
                                if (d[i] != "inherit") {
                                    f.fontWeight = d[i];
                                }
        ...
        svg.Element.radialGradient = function (node) {
            ...
            this.getGradient = function (ctx, element) {
                ...
                var r = (this.gradientUnits == "objectBoundingBox" ? (bb.width() + bb.height()) / 2 * this.attribute("r").numValue() : this.attribute("r").Length.toPixels());
            ...
            this.renderChildren = function (ctx) {
                ...
                for (var i = 0; i < this.children.length; i++) {
                    ...
                    if (textAnchor != "start" && (i == 0 || child.attribute("x").hasValue())) {
                        ...
                        child.x -= (textAnchor == "end" ? groupLength : groupLength / 2);
                    }
...
(function (Highcharts) {
    var UNDEFINED, DIV = "div", ABSOLUTE = "absolute", RELATIVE = "relative", HIDDEN = "hidden", VISIBLE = "visible", PX = "px", css = Highcharts.css, CanVGRenderer = Highcharts.CanVGRenderer, SVGRenderer = Highcharts.SVGRenderer, extend = Highcharts.extend, merge = Highcharts.merge, addEvent = Highcharts.addEvent, createElement = Highcharts.createElement, discardElement = Highcharts.discardElement;
    extend(CanVGRenderer.prototype, SVGRenderer.prototype);
    extend(CanVGRenderer.prototype, {create:function (chart, container, chartWidth, chartHeight) {
        ...
        var containerStyle = container.style, containerParent = container.parentNode, containerLeft = containerStyle.left, containerTop = containerStyle.top, containerOffsetWidth = container.offsetWidth, containerOffsetHeight = container.offsetHeight, canvas, initialHiddenStyle = {visibility:HIDDEN, position:ABSOLUTE};
        this.init.apply(this, [container, chartWidth, chartHeight]);
        canvas = createElement("canvas", {width:containerOffsetWidth, height:containerOffsetHeight}, {position:RELATIVE, left:containerLeft, top:containerTop}, container);
        this.canvas = canvas;
        ...
        var hiddenSvg = createElement(DIV, {width:containerOffsetWidth, height:containerOffsetHeight}, {visibility:HIDDEN, left:containerLeft, top:containerTop}, containerParent);
        this.hiddenSvg = hiddenSvg;
        hiddenSvg.appendChild(this.box);
    }, configure:function (chart) {
        var renderer = this, options = chart.options.tooltip, borderWidth = options.borderWidth, tooltipDiv = renderer.ttDiv, tooltipDivStyle = options.style, tooltipLine = renderer.ttLine, padding = parseInt(tooltipDivStyle.padding, 10);
        tooltipDivStyle = merge(tooltipDivStyle, {padding:padding + PX, "background-color":options.backgroundColor, "border-style":"solid", "border-width":borderWidth + PX, "border-radius":options.borderRadius + PX});
        if (options.shadow) {
            tooltipDivStyle = merge(tooltipDivStyle, {"box-shadow":"1px 1px 3px gray", "-webkit-box-shadow":"1px 1px 3px gray"});
        }
        css(tooltipDiv, tooltipDivStyle);
        css(tooltipLine, {"border-left":"1px solid darkgray"});
        addEvent(chart, "tooltipRefresh", function (args) {
            var chartContainer = chart.container, offsetLeft = chartContainer.offsetLeft, offsetTop = chartContainer.offsetTop, position;
            tooltipDiv.innerHTML = args.text;
            position = chart.tooltip.getPosition(tooltipDiv.offsetWidth, tooltipDiv.offsetHeight, {plotX:args.x, plotY:args.y});
            css(tooltipDiv, {visibility:VISIBLE, left:position.x + PX, top:position.y + PX, "border-color":args.borderColor});
            css(tooltipLine, {visibility:VISIBLE, left:offsetLeft + args.x + PX, top:offsetTop + chart.plotTop + PX, height:chart.plotHeight + PX});
