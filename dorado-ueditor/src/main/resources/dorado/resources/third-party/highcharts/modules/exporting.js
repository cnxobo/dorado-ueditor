...
(function (f) {
    ...
    x.exporting = {type:"image/png", url:"http://export.highcharts.com/", buttons:{contextButton:{menuClassName:"highcharts-contextmenu", symbol:"menu", _titleKey:"contextButtonTitle", menuItems:[{textKey:"printChart", onclick:function () {
        this.print();
    ...
    p(A.prototype, {getSVG:function (c) {
        ...
        d.destroy();
        n(c);
        b = b.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g, "").replace(/symbolName="[^"]+"/g, "").replace(/jQuery[0-9]+="[^"]+"/g, "").replace(/url\([^#]+#/g, "url(#").replace(/<svg /, "<svg xmlns:xlink=\"http://www.w3.org/1999/xlink\" ").replace(/ href=/g, " xlink:href=").replace(/\n/, " ").replace(/<\/svg>.*?$/, "</svg>").replace(/&nbsp;/g, "\xa0").replace(/&shy;/g, "").replace(/<IMG /g, "<image ").replace(/height=([^" ]+)/g, "height=\"$1\"").replace(/width=([^" ]+)/g, "width=\"$1\"").replace(/hc-svg-href="([^"]+)">/g, "xlink:href=\"$1\"/>").replace(/id=([^" >]+)/g, "id=\"$1\"").replace(/class=([^" >]+)/g, "class=\"$1\"").replace(/ transform /g, " ").replace(/:(path|rect)/g, "$1").replace(/style="([^"]+)"/g, function (a) {
            return a.toLowerCase();
        });
        return b = b.replace(/(url\(#highcharts-[0-9]+)&quot;/g, "$1").replace(/&quot;/g, "'");
    }, exportChart:function (c, a) {
        var c = c || {}, d = this.options.exporting, d = this.getSVG(o({chart:{borderRadius:0}}, d.chartOptions, a, {exporting:{sourceWidth:c.sourceWidth || d.sourceWidth, sourceHeight:c.sourceHeight || d.sourceHeight}})), c = o(this.options.exporting, c);
        ...
        if (!i) {
            e[o] = i = k("div", {className:c}, {position:"absolute", zIndex:1000, padding:s + "px"}, e.container), v = k("div", null, p({MozBoxShadow:"3px 3px 10px #888", WebkitBoxShadow:"3px 3px 10px #888", boxShadow:"3px 3px 10px #888"}, j.menuStyle), i), w = function () {
                u(i, {display:"none"});
                g && g.setState(0);
                e.openMenu = !1;
            ...
            }), e.exportDivElements.push(v, i), e.exportMenuWidth = i.offsetWidth, e.exportMenuHeight = i.offsetHeight;
        }
        a = {display:"block"};
