...
(function () {
    var g = $G, ajax = parent.baidu.editor.ajax, maskIframe = g("maskIframe");
    ...
    imageUploader.init = function (opt, callbacks) {
        ...
        if (srcImg) {
            ...
            var tabElements = g("imageTab").children, tabHeads = tabElements[0].children, tabBodys = tabElements[1].children;
    ...
    function searchImage() {
        ...
        var key = imgSearchInput.value, type = $G("imgType").value, keepOriginName = editor.options.keepOriginName ? "1" : "0", url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word=" + encodeToGb2312(key) + type + "&keeporiginname=" + keepOriginName + "&" + +new Date;
    ...
    function addScrollListener() {
        g("imageList").onscroll = function () {
            var imgs = this.getElementsByTagName("img"), top = Math.ceil(this.scrollTop / 100) - 1;
            top = top < 0 ? 0 : top;
    ...
    function insertSingle() {
        var url = g("url"), width = g("width"), height = g("height"), border = g("border"), vhSpace = g("vhSpace"), title = g("title"), align = findFocus("remoteFloat", "name"), imgObj = {};
        if (!url.value) {
            return;
        }
        if (!flagImg) {
    ...
    function addSizeChangeListener(percent) {
        var width = g("width"), height = g("height"), lock = g("lock");
    ...
    function createPreviewImage(url) {
        ...
        var img = document.createElement("img"), preview = g("preview");
        var imgTypeReg = /\.(png|gif|jpg|jpeg)$/gi, urlFilter = "";
    ...
    function showPreviewImage(img, needClone) {
        ...
        var maxWidth = 262, maxHeight = 262, target = scaling(tmpWidth, tmpHeight, maxWidth, maxHeight);
    ...
    function scaling(width, height, maxWidth, maxHeight) {
        ...
        var srcRatio = (width / height).toFixed(2), tarRatio = (maxWidth / maxHeight).toFixed(2), w, h;
    ...
    function createFlash(opt, callbacks) {
        ...
        var option = {createOptions:{id:"flash", url:opt.flashUrl, width:opt.width, height:opt.height, errorMessage:lang.flashError, wmode:browser.safari ? "transparent" : "window", ver:"10.0.0", vars:opt, container:opt.container}};
    ...
    function createAlignButton(ids) {
        for (var i = 0, ci; ci = ids[i++]; ) {
            var floatContainer = g(ci), nameMaps = {"none":lang.floatDefault, "left":lang.floatLeft, "right":lang.floatRight, "center":lang.floatCenter};
    ...
    function clickHandler(tabHeads, tabBodys, obj) {
        ...
        for (var j = 0, length = tabBodys.length; j < length; j++) {
            var body = tabBodys[j], id = body.getAttribute("id");
            ...
            if (id != tabSrc) {
                ...
                if (id == "imgManager") {
                    ...
                    if (!list.children.length) {
                        ajax.request(editor.options.imageManagerUrl, {timeout:100000, action:"get", onsuccess:function (xhr) {
                            var tmp = utils.trim(xhr.responseText), imageUrls = !tmp ? [] : tmp.split("ue_separate_ue"), length = imageUrls.length;
    ...
    function switchSelect(selectParentId) {
        var select = g(selectParentId), children = select.children;
