...
function getUserConfig() {
    var form = document.forms["data-form"], info = {title:form["title"].value, subTitle:form["sub-title"].value, xTitle:form["x-title"].value, yTitle:form["y-title"].value, suffix:form["unit"].value, tableDataFormat:getTableDataFormat(), tip:$("#tipInput").val()};
    return info;
}
...
function getSeriesAndCategories() {
    var form = document.forms["data-form"], series = [], categories = [], tmp = [], tableData = getTableData();
...
function updateConfigItem(value) {
    var table = $("#showTable")[0], isDisable = value === "disable" ? true : false;
...
function getSeriesForPieChart() {
    ...
    for (var j = 1, jlen = tableData[0].length; j < jlen; j++) {
        var title = tableData[0][j], val = tableData[1][j];
        series.data.push([title, val]);
    }
    return {series:[series]};
}
function getTableData() {
    var table = document.getElementById("showTable"), xCount = table.rows[0].cells.length - 1, values = getTableInputValue();
...
function getTableInputValue() {
    var table = document.getElementById("showTable"), inputs = table.getElementsByTagName("input"), values = [];
